...
export default function ClientsPage() {
  ...
  const handleSaveClient = async (
    ...
    if (clientData.id) {
      ...
      setClientList((prev) =>
        prev.map((c) =>
          c.id === clientData.id
            ? { ...c, name: clientData.name, description: clientData.description }
            : c
        )
      );
    } else {
      try {
        ...
        // Optional: Update context if it has an addClient method
        // addClientToContext(newClient);
        
      } catch (err) {
        console.error("Network error:", err);
        alert('Network error: Failed to add client');
      }
    }
  };
  ...
  if (loading) {
    return <p className="p-6">Chargement des clients...</p>;
  }
